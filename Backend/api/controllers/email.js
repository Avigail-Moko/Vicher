  ...
  verifyEmail: async (req, res) => {
    ...
    try {
      const decoded = jwt.verify(token, process.env.JWT_KEY);
       ...
       if (existingUser) {return res.status(200).json({ message: 'Email already verified. You can log in.' });}
       
      const pending = await PendingUser.findOne({ email: decoded.email });
      if (!pending) return res.status(404).json({ message: 'Pending user not found' });
      ...
      res.status(200).json({ message: 'Email verified. User created successfully.' });
    } catch (err) {
